...
class CrudCreate extends WebService
{
	...
	*/	
	public function injectDoctype($xmlDoc)
	{
		...
		$xmlDoc = substr($xmlDoc, 0, $posHeader)."\n<!DOCTYPE resultset PUBLIC \"-//Structured Dynamics LLC//Crud Create DTD 0.1//EN\" \"".$this->dtdBaseURL.$this->dtdURL."\">".substr($xmlDoc, $posHeader, strlen($xmlDoc) - $posHeader);	
			...
			if($this->conneg->getStatus() == 200)
			{
				...
				if($this->mode == "full" || $this->mode == "triplestore")
				{
					...
					if($datasetUri != "")
					{
						...
						$this->db->query("DB.DBA.RDF_LOAD_RDFXML_MT('".str_replace("'", "\'", $rdfxmlSerializer->getSerializedIndex($datasetRes))."', '".$this->wsf_graph."datasets/', '".$this->wsf_graph."datasets/')");		
