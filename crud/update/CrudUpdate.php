...
class CrudUpdate extends WebService
{
	...
	*/	
	public function injectDoctype($xmlDoc)
	{
		...
		$xmlDoc = substr($xmlDoc, 0, $posHeader)."\n<!DOCTYPE resultset PUBLIC \"-//Structured Dynamics LLC//Crud Update DTD 0.1//EN\" \"".$this->dtdBaseURL.$this->dtdURL."\">".substr($xmlDoc, $posHeader, strlen($xmlDoc) - $posHeader);	
...
//		$ws_dr = new DatasetRead($this->dataset, "false", $this->registered_ip, $this->requester_ip);
		...
		if($ws_dr->pipeline_getResponseHeaderStatus() != 200)
		{
			...
			$this->conneg->setError($ws_dr->pipeline_getError()->id, 
												$ws_dr->pipeline_getError()->webservice, 
												$ws_dr->pipeline_getError()->name, 
												$ws_dr->pipeline_getError()->description, 
												$ws_dr->pipeline_getError()->debugInfo,
												$ws_dr->pipeline_getError()->level);				
			return;
		}
	}
			...
			if($this->conneg->getStatus() == 200)
			{
				...
				if($this->mime != "application/rdf+xml" && $this->mime != "application/rdf+n3")
				{
					...
					$this->conneg->setError($this->errorMessenger->_306->id, 
														...
														"Can't create data of format: ".$this->mime,
				...
				@$this->db->query("DB.DBA.RDF_LOAD_RDFXML_MT('".str_replace("'", "\'", $rdfxmlSerializer->getSerializedIndex($irs))."', '$tempGraphUri', '$tempGraphUri', 0)");	
				...
				if(count($statementsUri) > 0)
				{
					...
					@$this->db->query("DB.DBA.RDF_LOAD_RDFXML_MT('".str_replace("'", "\'", $rdfxmlSerializer->getSerializedIndex($statements))."', '$tempGraphReificationUri', '$tempGraphReificationUri', 0)");	
...
*/
				...
				$labelProperties = array(	Namespaces::$iron."prefLabel",
				...
				$descriptionProperties = array(	Namespaces::$iron."description",
				...
				foreach($irsUri as $subject)
				{
					...
					foreach($labelProperties as $property)
					{
						...
						{
							$prefLabelFound = TRUE;
							$add .= "<field name=\"prefLabel\">".$this->xmlEncode($resourceIndex[$subject][$property][0]["value"])."</field>";
					...
					foreach($descriptionProperties as $property)
					{
						...
						{
							$add .= "<field name=\"description\">".$this->xmlEncode($resourceIndex[$subject][$property][0]["value"])."</field>";
					...
					{
						$add .= "<field name=\"prefURL\">".$this->xmlEncode($resourceIndex[$subject][$iron."prefURL"][0]["value"])."</field>";
					...
					foreach($resourceIndex[$subject] as $predicate => $values)
					{
						if(	array_search($predicate, $labelProperties) === FALSE &&
							...
							$predicate != Namespaces::$iron."prefURL") // skip label & description & prefURL properties
						{
							foreach($values as $value)
							{
								if($value["type"] == "literal")
								{					  
									$add .= "<field name=\"".urlencode($predicate)."_attr\">".$this->xmlEncode($value["value"])."</field>";
									...
									{							
										if($resourceIndex[$statementUri]["http://www.w3.org/1999/02/22-rdf-syntax-ns#subject"][0]["value"] == $subject &&
										   $resourceIndex[$statementUri]["http://www.w3.org/1999/02/22-rdf-syntax-ns#predicate"][0]["value"] == $predicate && 
										   $resourceIndex[$statementUri]["http://www.w3.org/1999/02/22-rdf-syntax-ns#object"][0]["value"] == $value["value"])
										{
								...
								elseif($value["type"] == "uri")
								{
									...
									if($labels != "")
									{
										$add .= "<field name=\"".urlencode($predicate)."_attr_obj\">".$this->xmlEncode($labels)."</field>";
										$add .= "<field name=\"".urlencode($predicate)."_attr_obj_uri\">".$this->xmlEncode($value["value"])."</field>";
										$add .= "<field name=\"attribute\">".$this->xmlEncode($predicate)."</field>";
									}
